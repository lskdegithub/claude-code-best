...
> {
  try {
    ...
    logForDebugging(
      `[OpenAI] Calling model=${openaiModel}, messages=${openaiMessages.length}, tools=${openaiTools.length}, max_tokens=${maxOutputTokens}`,
    )

    // 6. Call OpenAI API with streaming
    const stream = await createChatCompletionStreamWithTokenRecovery({
      client,
