...
import { calculateContextPercentages, getRawContextWindowForModel } from '../utils/context.js'
...
function buildStatusLineCommandInput(
  ...
  const rawContextWindowSize = getRawContextWindowForModel(
    runtimeModel,
    getSdkBetas(),
  )
  const configuredContextWindowSize = getConfiguredContextWindowSize(
    runtimeModel,
  )
  const contextWindowSize = getEffectiveContextWindowSize(runtimeModel)
  ...
  return {
    ...
    context_window: {
      ...
      configured_context_window_size: configuredContextWindowSize,
