...
function checkLargeToolResults(
  ...
  for (const tool of data.messageBreakdown.toolCallsByType) {
    ...
    const percent = (totalToolTokens / data.maxTokens) * 100
...
function checkReadResultBloat(
  ...
  const totalReadPercent = (totalReadTokens / data.maxTokens) * 100
  const readPercent = (readTool.resultTokens / data.maxTokens) * 100
...
function checkMemoryBloat(
  ...
  const memoryPercent = (totalMemoryTokens / data.maxTokens) * 100
