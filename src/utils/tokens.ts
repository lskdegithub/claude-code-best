...
/**
 * Conservative token estimate for threshold checks. tokenCountWithEstimation()
 * is usually more accurate because it anchors on the last real API usage, but
 * it can lag when we append synthetic/meta messages between turns. Taking the
 * max with a full rough pass avoids missing a context overflow by a small margin.
 */
export function tokenCountForThresholdChecks(
  messages: readonly Message[],
): number {
  const estimatedFromUsage = tokenCountWithEstimation(messages)
  const roughEstimate = roughTokenCountEstimationForMessages(
    messages as Parameters<typeof roughTokenCountEstimationForMessages>[0],
  )
  return Math.max(estimatedFromUsage, roughEstimate)
}
